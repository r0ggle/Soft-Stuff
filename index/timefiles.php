...
<?php
	} // list times from today, latest first
	$r = $mysqli->query("SELECT ta.name, ta.color, DATE_FORMAT(ti.time_start, '%a, %b %d, %H:%i') AS time_start, DATE_FORMAT(ti.time_end, '%H:%i') AS time_end FROM tasks AS ta INNER JOIN times as ti ON ta.id=ti.task_id WHERE ta.user_id='$user_id' AND DATE(time_start)=CURDATE() ORDER BY ti.time_start DESC LIMIT 16");
	while ($row = $r->fetch_assoc()) {
